...
def read_pseudotime_from_file(filename):
  # 
  pt = pd.read_csv(filename, sep="\t", index_col=0, names=["pseudotime"], error_bad_lines=False)["pseudotime"]
  return(pt)
...
def gene_correlation(subset_index, pseudotime, feature):
  subset_index = [value for value in subset_index if value in pseudotime.index.get_values()]
...
def transcript_correlation(subset, pseudotime, exp, method, feature):
  subset = subset[subset.isin(pseudotime)]
  # transcripts = exp.columns.copy()
  transcripts = exp.columns.copy()[1:10]
  subsetc = exp[exp.index.isin(subset.index)]
  
  subsetc["pseudotime"] = subset
  print("transcript_correlation")
...
def get_correlation_with_pseudotime(pseudotime, exp, annotation, gene_trx_dic, cell_set_flag=None, feature = "gene", correlation_threshold = 0, method = "spearman"):
    ...
    if cell_set_flag == "ctrl" and feature == "gene":
        spearman = gene_correlation(pseudotime, pseudotime, exp, method, feature)
    elif cell_set_flag == "exp" and feature == "gene":
        spearman = gene_correlation(pseudotime, pseudotime, exp, method, feature)
    if cell_set_flag == "ctrl" and feature == "transcript":
        spearman = transcript_correlation(pseudotime, pseudotime, exp, method, feature)
    elif cell_set_flag == "exp" and feature == "transcript":
        spearman = transcript_correlation(pseudotime, pseudotime, exp, method, feature)
        IPython.embed()
    else:
