...
parser.add_argument("-f", "--feature", dest="feature", default = "g", help="feature of interest; either 'g' (gene) or 't' (transcript) depending on desired output", metavar="FEATURE", required=False)
...
def load_ctrl_pseudotimes(pseudotime_files, ctrl_pseudotime_files=None, expression_table=None, annotation=None, gene_trx_dic=None, feature="t", correlation_method = "spearman"):
  # ipdb.set_trace()
...
def load_exp_pseudotimes(pseudotime_files, cpt="none", expression_table=None, annotation=None, gene_trx_dic=None, feature="t", correlation_method = "spearman"):
  # ipdb.set_trace()
  ...
  if cpt == "none":
    ...
    corr = read_corr(pt, expression_table, annotation, gene_trx_dic, cell_set_flag="exp", feature="t", correlation_method="spearman")
    corr_columns = []
    ...
    pt = read_pts(pseudotime_files)
    corr = read_corr(pt, expression_table, annotation, gene_trx_dic, cell_set_flag="mix", feature="t", correlation_method="spearman")
    corr_columns = []
    for i in sorted(pt.keys()):
      corr_columns += [i+"_exp_corr"]
...
corr, pt = load_exp_pseudotimes(pseudotime_files, expression_table = expression_table, feature="t")
...
while True:
  ...
  if(action == "X"):
    ...
    ctrl_ptime = input("Which ctrl pseudotime would you like to correlate with? Leave blank if no control present. ")
